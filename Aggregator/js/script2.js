...
$.ajax({
  ...
  for (var k = 0; k < 250; k++) {
    ...
    var $div = $("<div>", {class: "group", id: "div" + k});
    ($div).addClass("unclicked");

    var $img = $("<img>");
    
    var image_number = Math.floor(Math.random() * (img.length));
    $img.attr("src", img[image_number]);

    $div.append('<br>' + flavor1 + ' ');
    ...
    for (var i = start_number; i < (number_of_words + start_number); i++) {
      /*if (i == number_of_words + start_number - 1) {
        $('#div').append(words[i] + '<br>');
      }*/
      //else {
        $div.append(words[i] + ' ');//timeStamp + '<br>' + '<img src="' + photoURL + '">');
      //}
    }
...
function showPrice(item) {
  if ($(item).attr('class').indexOf("unclicked") > -1) {
    $(item).empty();
    var random_price = Math.floor(Math.random() * 3);
    random_price = Math.pow(10, random_price);
    random_price += Math.floor(Math.random() * random_price * 10);
    $(item).append('$' + random_price);
    $(item).css({
      'font-size' : '100%',
      'line-height': '290px'
    })
    $(item).removeClass('unclicked');
  }
}